...
export default function FavoritesPage() {
  ...
  if (sort === "오래된순") visible = [...visible].reverse();
            ...
            <div className="relative hidden shrink-0 lg:block">
              ...
              >
                ...
                <Icon
                  name="chevronRight"
                  size={12}
                  className={`rotate-90 transition-transform ${sortOpen ? "rotate-[270deg]" : ""}`}
                />
              </button>
              {sortOpen && (
                <div className="absolute right-0 top-7 z-10 w-28 overflow-hidden rounded-xl border border-line bg-white shadow-[0_6px_20px_rgba(30,39,97,0.12)]">
                  {SORTS.map((s) => (
                    <button
                      key={s}
                      onClick={() => {
                      ...
                      className={`block w-full px-4 py-2.5 text-left text-[13px] font-semibold ${
                        s === sort ? "bg-[#eef2fb] text-navy" : "text-navy-deep"
                      }`}
                    >
                      {s}
                    </button>
